...
```env
APP_ENV=development
DATABASE_URL=postgresql+psycopg://smart_garden:CHANGE_ME@localhost:5432/smart_garden
SECRET_KEY=CHANGE_ME
AUTH_TOKEN_TTL=3600
CORS_ORIGINS=http://localhost:3000
```

Создать локальный `.env` на основе примера и поставить собственные dev-значения. `.env` никогда не коммитить.
...
Создать локальную БД `smart_garden` и отдельного dev-пользователя `smart_garden`. Пароль выбрать локально и не отправлять в GitHub.

После создания проверить подключение через `psql` или другой PostgreSQL-клиент.

**Не создавать SQLite-файл. Не использовать `sqlite:///...`.**
