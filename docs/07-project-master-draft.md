...
**Кому читать:** новому участнику проекта и команде при восстановлении контекста/причин решений.  
**Статус:** историческая база знаний; для исполняемого кода приоритет имеют актуальный API Contract, GitHub Issue и Development Guide.
...
# 88. Решение: разработчики меняются Frontend / Backend ролями

**Дата:** 23 сентября 2026  
**Статус:** принятое решение.

Изначально Stage 1 был распределён:
- ONN9IX — Frontend;
- F1zname — Backend.

Уточнение: это **не постоянные специализации**.

На следующих этапах разработчики будут менять зоны ответственности. В частности, ONN9IX планирует на следующем этапе работать с Backend. F1zname сможет перейти на Frontend или другую назначенную часть.

Следствие: проект нельзя строить вокруг личного окружения одного человека.

---

# 89. Решение: никакого «пароля от Backend-разработчика»

Принято правило:

**у проекта не должно быть пароля/файла/настройки, которую новый Backend-разработчик обязан просить у предыдущего.**

Для этого:

- local PostgreSQL стандартизирована через Docker Compose;
- dev database имеет одинаковые локальные параметры;
- local database password — dev-only, не production secret;
- каждый разработчик создаёт собственный local SECRET_KEY;
- `.env.example` содержит полный шаблон;
- migrations находятся в Git;
- demo users создаются seed;
- README обязан описывать запуск;
- staging/production secrets в будущем хранятся централизованно;
- GitHub accounts остаются персональными.

Критерий качества передачи:
новый разработчик клонирует repository и поднимает нужную часть без вопроса предыдущему разработчику о секретах или локальной конфигурации.

---

# 90. Стандартизированная локальная PostgreSQL

Для dev принят `docker-compose.yml` в корне repository.
...
Важно:
- данные synthetic;
- порт привязан к localhost;
- dev-only пароль не считается production credential;
- тот же пароль нельзя переносить в staging/production;
- production DATABASE_URL не хранится в Git.
